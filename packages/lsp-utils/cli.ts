...
export const getCliConnection = (language: string): IConnection => {
  ...
  const cli = yargs
    ...
    .option(LspTransportMethod.IPC, {
      describe: `Use ${LspTransportMethod.IPC} to communicate with the server. Useful for calling from a node.js client`
    })
    ...
    .option(LspTransportMethod.PIPE, {
      describe: `Use a ${LspTransportMethod.PIPE} (with a name like --pipe=/tmp/named-pipe) to communicate with the server`,
      type: "string"
    })
    .option(LspTransportMethod.SOCKET, {
      describe: `Use a ${LspTransportMethod.SOCKET} (with a port number like --socket=5051) to communicate with the server`,
      type: "number"
    });
